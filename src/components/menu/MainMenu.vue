        ...
        <ul :class="['mainmenu', (animation ? 'animation' : '')]">
            ...
            <template v-for="( item, index ) in menu" :key="item">
                <li class="menu-item">
                    ...
                    <span class="link">
                        <component :is="htmlElement(item.route)" @click="(item.route ? closeMobileMenu() : '')" :to="{ name: item.route, params: { } }" :style="animation ? { animationDelay: `${ 1 + (index * 100)}ms` } : ''">
                        ...
                        <span class="icon" v-if="item.subItems" @click="toggleSubMenu(index)"><EpaIcon icon="angledown" /></span>
                        <template v-else></template>
                    </span>

                    <ul v-if="item.subItems && openSubMenu === index || item.subItems && location === 'header'" class="submenu">
                        <li class="subitem" v-for="sub in item.subItems" :key="sub">
                            <component :is="htmlElement(sub.route)" @click="closeMobileMenu()" :to="{ name: sub.route, params: { id: sub.id, slug: sub.slug } }">
                                {{ sub.name }}
                            </component>
                        </li>
                    </ul>
            ...
            <li :class="['menu-item', (animation ? 'fade' : '')]" :style=" animation ? { animationDelay: `${(menu.length + 2) * 100}ms` } : ''">
                <span class="link" >
                    <LanguageSwitcher/>
                </span>
            </li>
...
@keyframes menuitem { 0%{ transform: translateX(10rem); opacity: 0; } 100%{ transform: translateX(0); opacity: 1;} }
